...
import { addIDtoQuery, removeIDfromQuery, lengthOfQuery } from "../common/queryStrings.js";
...
const getMovieDetails = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `https://www.omdbapi.com/?i=${id}&type=movie&r=json&plot=short&apikey=112d4911`
    );
    dispatch({
...
const getMoviesLength = (id) => {
  lengthOfQuery("movies");
  return {
    type: types.GET_MOVIE_LENGTH,
    payload: id,
  };
};
